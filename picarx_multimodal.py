...
def multimodal(speed):
    ...
    grayscale_producer = Producer(grayin_bus.set_message(s.get_adc_value(),"grayscale_producer"), grayin_bus, delay = sensor_delay, name = "grayscale_producer")
    grayscale_cp = ConsumerProducer(grayout_bus.set_message(i.get_grayscale_value(grayin_bus.get_message("grayscale_cp")),"grayscale_cp"), grayin_bus, grayout_bus, delay = interpreter_delay, name = "grayscale_cp")
